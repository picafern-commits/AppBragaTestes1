...
(function(){
  ...
  function install(){
    unlockScroll();
    window.filtrarImpressoras = applyFilter;
    setTimeout(()=>renderAll(getData()),50);
    setTimeout(()=>{unlockScroll();renderAll(getData());},800);
    setTimeout(()=>{unlockScroll();renderAll(getData());},2000);
    setInterval(()=>{unlockScroll();renderAll(getData());},5000);
  }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded', install); else install();
})();
